...
export default function DashboardPage() {
  ...
  // Show habits that are scheduled for today (frequency habits always show)
  const todayHabits = habits.filter((h) => isScheduledDay(h.schedule, today));
  ...
  return (
    <div className="px-4 pt-14 safe-top max-w-lg mx-auto">
      {/* Header */}
      <div className="mb-6">
        <p className="text-white/40 font-body text-sm">{format(today, "EEEE, MMMM d")}</p>
      ...
      {todayHabits.length > 0 && (
        <DailyProgress completed={completedToday} total={todayHabits.length} allDone={allDone} />
      )}
      ...
      <div className="mt-6 space-y-3">
        {loading ? (
          ...
          <div className="text-center py-16">
            ...
            <p className="font-display text-white/60 font-600">No habits for today</p>
            <p className="text-white/30 text-sm mt-1">Add a habit to get started</p>
          </div>
        ) : (
